...
class ATOCRunner(Runner):
    ...
    def __init__(self, config:Dict):
        ...
        # init model 
        self.buff = ATOCReplayBuffer(self.capacity,self.observation_space,self.n_actions,self.n_ant)
        self.model = DGN(self.n_ant,self.observation_space,self.hidden_dim,self.n_actions)
        self.model_tar = DGN(self.n_ant,self.observation_space,self.hidden_dim,self.n_actions)
        ...
        self.M_Null = torch.Tensor(np.array([np.eye(self.n_ant)]*self.batch_size)).to(self.device)
        self.M_ZERO = torch.Tensor(np.zeros((self.batch_size,self.n_ant,self.n_ant))).to(self.device)
    ...
    def run(self):
        ...
        while i_episode<self.n_episode:
            ...
            while steps < self.episode_length:
                ...
                action=[]
                cost_all += adj.sum()
                v_a = np.array(self.att(torch.Tensor(np.array([obs])).to(self.device))[0].cpu().data)
                
                for i in range(self.n_ant):
                    if np.random.rand() < self.epsilon:
                        adj[i] = adj[i]*0 if np.random.rand() < 0.5 else adj[i]*1
                    else:
                        adj[i] = adj[i]*0 if v_a[i][0] < self.threshold else adj[i]*1
                n_adj = adj*self.comm_flag
                cost_comm += n_adj.sum()
                n_adj = n_adj + np.eye(self.n_ant)

                q = self.model(torch.Tensor(np.array([obs])).to(self.device), torch.Tensor(np.array([n_adj])).to(self.device))[0]
                for i in range(self.n_ant):
                    if np.random.rand() < self.epsilon:
                ...
                self.buff.add(np.array(obs),orig_action,reward,np.array(next_obs),n_adj,next_adj,terminated)
                obs = next_obs
                adj = next_adj
            ...
            train_infos = dict()  # since we don't have policy network in DGN, init to empty dict 
            ...
            if self.buff.len > self.batch_size:
                for e in range(self.n_epoch):
                    O,A,R,Next_O,Matrix,Next_Matrix,D = self.buff.getBatch(self.batch_size)
                    ...
                    label = self.model(Next_O, Next_Matrix+self.M_Null).max(dim = 2)[0] - self.model(Next_O, self.M_Null).max(dim = 2)[0]
                    ...
                    Next_Matrix = torch.where(V_A_D > self.threshold, Next_Matrix, self.M_ZERO)
                    ...
                    for j in range(self.batch_size):
                        for i in range(self.n_ant):
                            expected_q[j][i][A[j][i]] = R[j][i] + (1-D[j])*self.gamma*target_q_values[j][i]
                    
                    loss = (q_values - torch.Tensor(expected_q).to(self.device)).pow(2).mean()
                    ...
                    with torch.no_grad():
                        ...
                        for p, p_targ in zip(self.att.parameters(), self.att_tar.parameters()):
                            p_targ.data.mul_(self.tau)
                            p_targ.data.add_((1 - self.tau) * p.data)
    ...
    def run(self):
        ...
        while i_episode<self.n_episode:
            ...
            while steps < self.episode_length:
                steps+=1 
                action=[]
                q = self.model(torch.Tensor(np.array([obs])).to(self.device), torch.Tensor(adj).to(self.device))[0]
                for i in range(self.n_ant):
                    if np.random.rand() < self.epsilon:
                        a = np.random.randint(self.n_actions)
                    else:
                ...
                self.buff.add(np.array(obs),orig_action,reward,np.array(next_obs),adj,next_adj,terminated)
                obs = next_obs
                adj = next_adj
            ...
            if self.buff.num_experiences > self.batch_size:
                for e in range(self.n_epoch):
                    ...
                    q_values = self.model(torch.Tensor(self.O).to(self.device), torch.Tensor(self.Matrix).to(self.device))
                    target_q_values = self.model_tar(torch.Tensor(self.Next_O).to(self.device), torch.Tensor(self.Next_Matrix).to(self.device)).max(dim = 2)[0]
                    target_q_values = np.array(target_q_values.cpu().data)
                    expected_q = np.array(q_values.cpu().data)
