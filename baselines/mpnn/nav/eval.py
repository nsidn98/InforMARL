...
from util import normalize_obs
...
def evaluate(args, seed, policies_list, ob_rms=None, render=False, env=None, master=None, render_attn=True):
    ...
    for t in range(num_eval_episodes):
        ...
        while not np.all(done):
            ...
            # rearrange actions according to the action space (Discrete action space)
            # actions = np.eye(env.action_space[0].n)[actions]
            episode_steps += 1
