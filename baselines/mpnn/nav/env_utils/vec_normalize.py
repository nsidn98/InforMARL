...
class MultiAgentVecNormalize(VecEnvWrapper):
    ...
    def step_wait(self):
        ...
        """
        obs, rews, news, infos = self.venv.step_wait()
        orig_rews = rews.copy()    # keep original rewards in info dict for comparing same stuff as ours
        self.ret = self.ret * self.gamma + rews
        obs = self._obfilt(obs)
        if self.ret_rms:
            self.ret_rms.update(self.ret)
    ...
    def _obfilt(self, obs):
        if self.ob_rms:
            ...
            for j in range(self.n):
                ...
                t = np.clip((np.array(list(obs[:,j]),dtype=np.float) - 
                            self.ob_rms[j].mean) / np.sqrt(self.ob_rms[j].var + 
