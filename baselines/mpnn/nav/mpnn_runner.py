...
class Runner(object):
    ...
    def __init__(self, config:Dict):
        ...
        self.eval_master, self.eval_env = setup_master(self.args, return_env=True, device=self.device) 
    
    def save(self):
        savedict = {'models': [agent.actor_critic.state_dict() 
                                for agent in self.master.all_agents]}
        ob_rms = (None, None) if self.envs.ob_rms is None \
    ...
    def run(self): 
        ...
        episode_rewards = torch.zeros([self.args.n_rollout_threads, n], device=self.device)
        final_rewards = torch.zeros([self.args.n_rollout_threads, n], device=self.device)
        rewards_over_eps = np.zeros((self.args.episode_length, 
                                self.args.n_rollout_threads, 
                                self.args.num_agents, 1), 
                                dtype=np.float32)

        # start simulations
        start = datetime.datetime.now()
        episodes = int(self.args.num_env_steps) // self.args.episode_length // self.args.n_rollout_threads
        ...
        for episode in range(episodes):
            ...
            total_num_steps = (episode + 1) * self.args.episode_length * self.args.n_rollout_threads
            ...
            if self.args.eval_interval is not None and episode%self.args.eval_interval==0:
                ob_rms = (None, None) if self.envs.ob_rms is None \
                    else (self.envs.ob_rms[0].mean, self.envs.ob_rms[0].var)
                _, eval_perstep_rewards, final_min_dists, \
                    num_success, eval_episode_len = evaluate(self.args, None, 
                                                            self.master.all_policies,
                                                            ob_rms=ob_rms, env=self.eval_env,
