...
from baselines.offpolicy.algorithms.maddpg.algorithm.graph_actor_critic import GMADDPG_Actor, GMADDPG_Critic
from baselines.offpolicy.algorithms.matd3.algorithm.graph_actor_critic import GMATD3_Actor, GMATD3_Critic
from baselines.offpolicy.utils.util import is_discrete, is_multidiscrete, get_dim_from_space, DecayThenFlatSchedule, soft_update, hard_update, \
...
class GMADDPGPolicy(GraphMLPPolicy):
    ...
    def __init__(self,
        ...
        self.node_obs_space = policy_config["node_obs_space"] # TODO add node_obs_space and edge_obs_space to config
        ...
        self.actor = actor_class(args = self.args, 
                                obs_dim = self.obs_dim,
                                node_obs_space = self.node_obs_space,
                                edge_obs_space = self.edge_obs_space,
                                act_dim = self.act_dim, 
                                device = self.device)
        self.critic = critic_class(args = self.args, 
        ...
        self.target_actor = actor_class(args = self.args, 
                                obs_dim = self.obs_dim,
                                node_obs_space = self.node_obs_space,
                                edge_obs_space = self.edge_obs_space,
                                act_dim = self.act_dim, 
                                device = self.device)
        self.target_critic = critic_class(args = self.args, 
                                central_obs_dim = self.central_obs_dim, 
                                central_act_dim = self.central_act_dim,
                                node_obs_space = self.node_obs_space,
                                edge_obs_space = self.edge_obs_space, 
    ...
    def get_actions(self, obs, node_obs, adj, agent_id, 
                    available_actions=None, t_env=None, explore=False, use_target=False, use_gumbel=False):
        """See parent class."""
    ...
    def get_random_actions(self, obs, available_actions=None):
        ...
        if self.discrete:
            if self.multidiscrete:
                random_actions = [OneHotCategorical(logits=torch.ones(batch_size, 
                                self.act_dim[i])).sample().numpy() for i in
                                range(len(self.act_dim))]
                random_actions = np.concatenate(random_actions, axis=-1)
            else:
                if available_actions is not None:
                    logits = avail_choose(torch.ones(batch_size, self.act_dim), 
                                        available_actions)
                    random_actions = OneHotCategorical(logits=logits).sample().numpy()
                else:
