...
from baselines.offpolicy.algorithms.maddpg.algorithm.actor_critic import MADDPG_Actor, MADDPG_Critic
from baselines.offpolicy.algorithms.matd3.algorithm.actor_critic import MATD3_Actor, MATD3_Critic
...
class MADDPGPolicy(MLPPolicy):
    ...
    def __init__(self, config, policy_config, target_noise=None, td3=False, train=True):
        ...
        self.actor = actor_class(self.args, self.obs_dim, 
                                self.act_dim, self.device)
        self.critic = critic_class(self.args, self.central_obs_dim, 
                                self.central_act_dim, self.device)

        self.target_actor = actor_class(self.args, self.obs_dim, 
                                self.act_dim, self.device)
        self.target_critic = critic_class(self.args, self.central_obs_dim, 
                                self.central_act_dim, self.device)
