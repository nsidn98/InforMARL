import numpy as np
import torch
from baselines.offpolicy.algorithms.mqmix.algorithm.agent_q_function import AgentQFunction
from torch.distributions import Categorical, OneHotCategorical
from baselines.offpolicy.utils.util import get_dim_from_space, is_discrete, \
                        is_multidiscrete, make_onehot, DecayThenFlatSchedule, \
                        avail_choose, to_torch, to_numpy, get_dim_from_space
...
class M_QMixPolicy(MLPPolicy):
    ...
    def __init__(self, config, policy_config, train=True):
        ...
        self.output_dim = sum(self.act_dim) if isinstance(self.act_dim, np.ndarray) \
                                            else self.act_dim
        ...
        self.q_network = AgentQFunction(self.args, 
                                        self.q_network_input_dim, 
                                        self.act_dim, 
                                        self.device)
        ...
        if action_batch is not None:
            ...
            if self.multidiscrete:
                ...
                for i in range(len(self.act_dim)):
                    ...
                    curr_q_values = torch.gather(curr_q_batch, 1, 
                                                curr_action_batch.unsqueeze(dim=-1))
        ...
        if self.multidiscrete:
            ...
            for i in range(len(self.act_dim)):
                ...
                if explore:
                    ...
                    random_action = Categorical(logits=torch.ones(batch_size, 
                                    self.act_dim[i])).sample().numpy()
                    take_random = (rand_number < eps).astype(int)
                    action = (1 - take_random) * to_numpy(greedy_action) + \
            ...
            if explore:
                ...
                actions = (1 - take_random) * to_numpy(greedy_actions) + \
