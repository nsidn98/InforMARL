...
class M_QMix:
    def __init__(self, 
        ...
        self.policy_agents = {
            ...
            if self.policy_mapping_fn(agent_id) == policy_id]) for policy_id in \
            self.policies.keys()
                            }
        if self.use_popart:
        ...
        for p_id in self.policy_ids:
            ...
            if isinstance(pol_all_q_out, list):
                ...
                for i in range(len(policy.act_dim)):
                    ...
                    curr_pol_q_out = torch.gather(curr_all_q_out, 1, 
                                    curr_stacked_act_batch_ind.unsqueeze(dim=-1))
                ...
                pol_q_outs = torch.gather(pol_all_q_out, 1, 
                                    stacked_act_batch_ind.unsqueeze(dim=-1))
