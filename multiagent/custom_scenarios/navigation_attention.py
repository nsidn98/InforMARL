"""
    ...
    `simple_spread.py` where any agent can get to any goal (check `reward()`)
     ...
     NOTE: This is made to be compatible with the scalableMARL official implementation
     https://github.com/christopher-hsu/scalableMARL
"""
...
class Scenario(BaseScenario):
    ...
    def make_world(self, args:argparse.Namespace) -> World:
        ...
        self.num_scripted_agents_goals = self.num_scripted_agents
        world.collaborative = args.collaborative

        ################# setup from setTracking_v0 scenario ##################
        self.agent_dim = 3
        self.target_dim = 4
        ...
        world.scripted_agents_goals = [Landmark() for i in range(self.num_scripted_agents_goals)]
        ...
        for i, obstacle in enumerate(world.obstacles):
            ...
            global_id += 1
        # add belief targets
        # world.belief_targets = [KFbelief(agent_id = 'target-' + str(i),
    ...
    def random_scenario(self, world):
        ...
        # TODO set belief targets, avoid collision (refer to collision_func)
        for nn in range(self.num_landmarks):
    ...
    def graph_observation(self, agent:Agent, world:World) -> Tuple[arr, arr]:
        ...
        if self.degree == 0:
            ...
            neigh.fit(node_obs[:, 2:4]) # only consider agent_pos to connect to closest neighbours
