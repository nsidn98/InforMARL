    ...
    def step_wait(self):
        ...
        for (i, done) in enumerate(dones):
            if all(done): 
                ...
                self.ts[i] = 0
        self.actions = None
        return (np.array(obs), np.array(shared_obs), np.array(rews), 
...
class SubprocVecEnv(VecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        VecEnv.__init__(self, len(env_fns), observation_space, 
                        share_observation_space, action_space)
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=shareworker, args=(work_remote, remote, 
