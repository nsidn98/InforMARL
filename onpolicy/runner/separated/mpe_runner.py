...
class MPERunner(Runner):
            ...
            if episode % self.log_interval == 0:
                ...
                if self.env_name == "MPE":
                    ...
                    for agent_id in range(self.num_agents):
                        ...
                        avg_agent_ep_rew = np.mean(self.buffer[agent_id].rewards) * \
                                                    self.episode_length
                        train_infos[agent_id].update({'individual_rewards': np.mean(idv_rews)})
                        train_infos[agent_id].update({"average_episode_rewards": 
                                                    avg_agent_ep_rew})
                        avg_ep_rews.append(avg_agent_ep_rew)
                print(f"Average episode rewards is {np.sum(avg_ep_rews):.3f} \t"
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            eval_rnn_states[eval_dones == True] = np.zeros(((eval_dones == True).sum(), 
                                                            self.recurrent_N, 
                                                            self.hidden_size), 
                                                            dtype=np.float32)
        ...
        for agent_id in range(self.num_agents):
            ...
            print(f"Eval average episode rewards of agent_{agent_id}: " + str(eval_average_episode_rewards))
